...
import { POST_KIND_LABELS, getArticle, getArticleNeighbours, listArticleSlugs } from '@/lib/content';
import { absoluteUrl } from '@/lib/env';
import { formatLongDate, readingTimeMinutes } from '@/lib/format';
import { issueJsonLd } from '@/lib/structured-data';

/**
 * An article or long-term review. The same reading page as a briefing — the
 * same measure, type and print behaviour — with the kind named above the
 * title instead of an issue number, because these are not numbered.
 *
...
/** As for briefings: where the post exists on the publishing platform, that copy is canonical. */
function canonicalFor(issue: { slug: string; externalUrl: string | null }): string {
  return issue.externalUrl ?? absoluteUrl(`/articles/${issue.slug}`);
}
...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      ...
      title: issue.title,
      description: issue.excerpt ?? publication.description,
      url: absoluteUrl(`/articles/${issue.slug}`),
...
export default async function ArticlePage(props: PageProps<'/articles/[slug]'>) {
  ...
  const { previous, next } = await getArticleNeighbours(slug);
  const date = formatLongDate(issue.publishedAt);
  const kindLabel = POST_KIND_LABELS[issue.kind];
    ...
    <article>
      ...
      <Container width="reading" className="pt-10 sm:pt-14">
        <p className="text-meta text-muted">
          <TextLink href="/articles" className="no-underline hover:underline">
            Articles
          </TextLink>
        </p>

        <p className="kicker mt-6">{kindLabel}</p>
        <h1 className="mt-2 text-title font-semibold text-fg">{issue.title}</h1>
      ...
      <Container width="reading" className="mt-12">
        {issue.contentHtml ? (
          ...
          <p className="max-w-measure text-muted">
            The text of this {kindLabel.toLowerCase()} is not stored here.
            {issue.externalUrl ? (
              <>
                {' '}
        ...
        <PrintPermalink path={`/articles/${issue.slug}`} className="mt-12" />
      </Container>
      ...
      {previous || next ? (
        <Container width="reading" className="mt-16">
          {/* Hidden in print: both sides are links to pages a sheet of paper
              cannot reach. */}
          <nav aria-label={`More ${kindLabel.toLowerCase()}s`} className="border-t border-hairline pt-8 print:hidden">
            {/* Position encodes direction: earlier on the left, later on the
                right. No arrow glyphs. */}
            <div className="grid gap-8 sm:grid-cols-2">
              <AdjacentIssue issue={previous} label={`Previous ${kindLabel.toLowerCase()}`} align="start" />
              <AdjacentIssue issue={next} label={`Next ${kindLabel.toLowerCase()}`} align="end" />
            </div>
          </nav>
        </Container>
      ) : null}
